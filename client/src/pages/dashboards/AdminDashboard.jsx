    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#080E1E] pt-28 pb-20 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white">Admin Platform Command Center</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">CareSync system telemetry, user controls, and doctor verifications.</p>
          </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div className="bg-white dark:bg-[#111B33] p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-extrabold text-slate-900 dark:text-white">{stats?.totalPatients || 0}</p>
                </div>
              ...
              <div className="bg-white dark:bg-[#111B33] p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between">
                <div>
                  <span className="text-xs font-semibold text-slate-400 dark:text-slate-400 block mb-1">Verified Doctors</span>
                  <p className="text-2xl font-extrabold text-cyan-600 dark:text-cyan-400">{stats?.totalDoctors || 0}</p>
                </div>
              ...
              <div className="bg-white dark:bg-[#111B33] p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between">
                <div>
                  <span className="text-xs font-semibold text-slate-400 dark:text-slate-400 block mb-1">Appointments</span>
                  <p className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">{stats?.totalAppointments || 0}</p>
                </div>
              ...
              <div className="bg-white dark:bg-[#111B33] p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between">
                <div>
                  <span className="text-xs font-semibold text-slate-400 dark:text-slate-400 block mb-1">Platform Revenue</span>
                  <p className="text-2xl font-extrabold text-slate-900 dark:text-white">${stats?.totalRevenue || 0}</p>
                </div>
            ...
            {/* Growth Graph */}
            <div className="bg-white dark:bg-[#111B33] p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm mb-8">
              <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-4">Platform User & Booking Growth</h3>
              <div className="h-64">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {doctors.map(doc => (
                        <tr key={doc._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-850/50">
                          <td className="py-3 px-2 font-bold text-slate-900 dark:text-white">{doc.name}</td>
                          <td className="py-3 px-2 text-slate-600 dark:text-slate-300">{doc.specialization}</td>
                          <td className="py-3 px-2 font-bold text-slate-800 dark:text-slate-200">${doc.consultationFee}</td>
                          <td className="py-3 px-2 text-slate-700 dark:text-slate-300">★ {doc.rating}</td>
                          <td className="py-3 px-2">
                            {doc.isVerified ? (
                              ...
                              <span className="px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 font-bold text-[10px]">
                                Pending
                              </span>
                            )}
                          </td>
                ...
                <div className="space-y-4">
                  <div className="relative max-w-sm">
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-850 text-slate-900 dark:text-white placeholder:text-slate-400 text-xs focus:outline-none focus:border-brand-600 dark:focus:border-cyan-400"
                    />
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {filteredUsers.map(u => (
                          <tr key={u._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-850/50">
                            ...
                            <td className="py-3 px-2 text-slate-600 dark:text-slate-300">{u.email}</td>
                            <td className="py-3 px-2">
                              <span className="px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold uppercase text-[10px]">
                                {u.role}
                              </span>
                            </td>
