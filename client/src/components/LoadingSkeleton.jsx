...
export function DoctorSkeleton() {
  return (
    <div className="bg-white rounded-3xl p-5 border border-slate-200 animate-pulse">
      <div className="w-full aspect-[4/3] bg-slate-200 rounded-2xl mb-4" />
  ...
  return (
    <div className="space-y-6 animate-pulse">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-28 bg-white rounded-3xl border border-slate-200 p-6" />
        ))}
      </div>
      <div className="h-80 bg-white rounded-3xl border border-slate-200 p-6" />
    </div>
  );
}
