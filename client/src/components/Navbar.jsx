...
import { Heart, Activity, User, LogOut, LayoutDashboard, Menu, X, Shield, Calendar, Stethoscope } from 'lucide-react';
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2.5 group">
            ...
            <div className="flex flex-col">
              <span className="text-xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-slate-900 via-brand-900 to-brand-600">
            ...
            {isAuthenticated ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-3 p-1.5 pl-3 rounded-full border border-slate-200 bg-white hover:bg-slate-50 transition-all shadow-sm"
                >
                  ...
                  {userDropdownOpen && (
                    ...
                    >
                      <div className="px-3 py-2 border-b border-slate-100 mb-1">
                        ...
                        <span className="inline-block mt-1 px-2 py-0.5 bg-brand-50 text-brand-700 text-[10px] font-semibold rounded-full uppercase">
                          {user?.role}
                        </span>
                      </div>
                      ...
                      <Link
                        ...
                        className="flex items-center gap-2.5 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 rounded-xl transition-colors font-medium"
                      >
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-rose-600 hover:bg-rose-50 rounded-xl transition-colors font-medium mt-1"
                      >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-white/95 backdrop-blur-xl border-b border-slate-200 px-6 py-4"
          >
            <div className="flex flex-col gap-3">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="px-3 py-2 text-base font-medium text-slate-700 hover:bg-slate-50 rounded-xl"
                >
                  {link.name}
                </Link>
              ))}
              ...
              {isAuthenticated ? (
                <>
                  <Link
                    to={getDashboardRoute()}
                    onClick={() => setMobileMenuOpen(false)}
                    className="px-3 py-2 text-base font-semibold text-brand-600 hover:bg-brand-50 rounded-xl flex items-center gap-2"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-2 text-base font-semibold text-rose-600 hover:bg-rose-50 rounded-xl flex items-center gap-2 text-left"
                  >
                ...
                <div className="flex flex-col gap-2 pt-2">
                  <Link
                    to="/login"
                    onClick={() => setMobileMenuOpen(false)}
                    className="w-full text-center py-2.5 text-base font-semibold text-slate-700 border border-slate-200 rounded-xl"
                  >
                    Sign In
                  </Link>
                  <Link
