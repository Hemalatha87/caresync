...
function MedicalCross(props) {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.rotation.y = THREE.MathUtils.lerp(meshRef.current.rotation.y, targetX + state.clock.getElapsedTime() * 0.2, 0.05);
    }
  });
...
function OrbitingRings() {
  ...
  useFrame((state) => {
    ...
    if (ring2.current) {
      ...
      ring2.current.rotation.z = Math.cos(t * 0.2) * 0.5;
    }
  });
...
// DNA / Healthcare Floating Spheres
...
export default function HeroScene() {
  return (
    <div className="w-full h-[480px] lg:h-[580px] relative cursor-grab active:cursor-grabbing">
      <Canvas
      ...
      >
        ...
        <directionalLight position={[5, 8, 5]} intensity={1.5} color="#F0F9FF" />
        <pointLight position={[-5, -5, -2]} intensity={1} color="#06B6D4" />
        
        <Sparkles count={40} scale={8} size={2.5} speed={0.4} opacity={0.6} color="#38BDF8" />
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 bg-brand-500/20 rounded-full blur-3xl pointer-events-none -z-10" />
    </div>
  );
}
